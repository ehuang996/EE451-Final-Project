...
in C++17 and parallelized with OpenMP and POSIX threads. KNN and MLP now use
vendor BLAS kernels for their dense inner loops.
...
│   │   ├── mlp/
│   │   │   ├── mlp.cpp        MLP, BCE + L2, BLAS batch backend
...
```bash
sbatch slurm/job.sl       # outputs combined run to alljob.out
```
...
```bash
sbatch slurm/job_analytics.sl     # outputs to analyticsjob.out
...
KNN and MLP now require BLAS on CARC. The shipped scripts build them that way
by default and use different runtime thread policies:
- KNN: outer OpenMP/pthreads own query parallelism, BLAS pinned to 1 thread.
- MLP: BLAS owns the dense minibatch kernels and gets the full thread budget.

```bash
sbatch src/cpp/knn/job_knn.sl
sbatch src/cpp/mlp/job_mlp.sl
...
KNN requires a BLAS build. The recommended runtime keeps BLAS at one thread and
lets our code own outer query parallelism:
...
Use `KNN_BLAS_BLOCK=<queries>` to tune SGEMM query-block size.

MLP also requires a BLAS build. Unlike KNN, the BLAS path owns the dense
minibatch kernels directly, so the recommended run mode is to hand the thread
budget to the BLAS library rather than pinning it to 1:
...
Use `MLP_BLAS_BLOCK=<rows>` to tune batched BLAS prediction blocking.
...
**macOS note:** Apple Clang ships without OpenMP. Use Homebrew GCC
(`brew install gcc`) or install `libomp` and pass
`-Xpreprocessor -fopenmp -lomp` to Apple Clang. Easier path: just run on CARC.
