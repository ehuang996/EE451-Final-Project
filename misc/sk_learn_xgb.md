...
| MLP  | 103→64→32→1, ReLU/sigmoid, BCE, SGD+momentum=0.9, batch=128, 30 epochs, `lr=0.01 / √(epoch+1)` (pthreads + OMP) | `MLPClassifier(hidden_layer_sizes=(64,32), activation='relu', solver='sgd', learning_rate_init=0.01, learning_rate='invscaling', momentum=0.9, alpha=1e-4, batch_size=128, max_iter=30, nesterovs_momentum=False)` | — | **BLAS-implicit** — parallelism lives inside the BLAS GEMM calls, pinned via `OMP_NUM_THREADS=8` before import |
